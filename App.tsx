    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        if (localResult.isPrintable) {
          ...
          setFileError("Design integrity failed. Details are too fine for 3D printing.");
          setActiveStep(1);
        }
      } catch (err) {
        setFileError("Analysis failed.");
        setActiveStep(1);
      } finally {
        setProcessingStatus("");
      }
                  ...
                  {adminDesigns.map(d => (
                    <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-4 text-xs">
                        ...
                        <span className="block text-slate-400">{d.analysis.suggestedColors.length} Colors</span>
                      </td>
                      <td className="p-4">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
              <div className="hidden md:flex gap-8 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">
                 <span className={activeStep >= 1 ? 'text-slate-900' : ''}>01 Create</span>
                 <span className={activeStep >= 3 ? 'text-slate-900' : ''}>02 Configure</span>
                 <span className={activeStep === 4 ? 'text-slate-900' : ''}>03 Checkout</span>
              </div>
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-green-600 bg-green-50 px-3 py-1.5 rounded-full border border-green-100">
                <ShieldCheck size={14} /> Local Engine
              </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-7 space-y-6">
          {logoConfig.url ? (
            ...
            <div className="w-full h-[450px] rounded-[2rem] bg-white border border-slate-200 shadow-soft flex flex-col items-center justify-center text-slate-300 relative overflow-hidden">
                ...
                <div className="bg-slate-50 p-6 rounded-full mb-4 animate-bounce-slow"><Upload size={40} className="text-slate-200" /></div>
                <p className="font-medium text-slate-400">Upload your brand logo to start</p>
            </div>
          )}

          {/* AI CAPABILITY PANEL */}
          {analysis && analysis.capabilities && (
            <div className="bg-white border border-slate-100 rounded-[2rem] p-8 shadow-soft animate-fade-in">
              <div className="flex items-center gap-2 mb-6">
                <div className="p-2 bg-blue-50 text-blue-600 rounded-lg"><Cpu size={18}/></div>
                <h3 className="font-bold text-sm tracking-tight uppercase">AI Vision Engine Report</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {analysis.capabilities.map((cap, i) => (
                  <div key={i} className="space-y-2">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold uppercase text-slate-400">{cap.title}</span>
                        {cap.status === 'optimal' ? <CheckCircle2 size={12} className="text-green-500" /> : <AlertTriangle size={12} className="text-amber-500" />}
                    </div>
                    <p className="text-[11px] leading-relaxed text-slate-600 font-medium">{cap.description}</p>
                    <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full ${cap.status === 'optimal' ? 'bg-green-500' : 'bg-amber-500'}`} 
                          style={{width: cap.status === 'optimal' ? '100%' : '60%'}}
                        ></div>
                    </div>
                  </div>
                ))}
        ...
        <div className="lg:col-span-5 space-y-8">
          {activeStep <= 2 && (
            <div className="bg-white p-8 rounded-[2rem] shadow-card border border-slate-100 relative overflow-hidden">
              <div className="relative z-10">
                <h2 className="text-xl font-bold mb-2">Initialize Project</h2>
                <p className="text-slate-400 text-sm mb-6">Transforming your 2D assets into printable 3D geometry.</p>
                ...
                {activeStep === 1 ? (
                  <div className="group relative h-48 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center bg-slate-50 hover:bg-orange-50 hover:border-orange-200 transition-all cursor-pointer">
                    ...
                    <span className="text-sm font-bold mt-4 text-slate-600">Drop your logo here</span>
                    <span className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider">Automated vector conversion</span>
                  </div>
                ) : (
                  <div className="text-center py-12">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Analyzing structural feasibility...</p>
                  </div>
                )}
                {fileError && <p className="text-red-500 text-xs mt-4 font-medium flex items-center gap-1"><AlertTriangle size={14}/> {fileError}</p>}
              </div>
            </div>
          )}
          ...
          {activeStep >= 3 && analysis && (
            <div className="bg-white p-8 rounded-[2rem] shadow-card border border-slate-100 space-y-8 animate-fade-in">
              <div className="flex justify-between items-start">
                <div>
                    <h2 className="font-bold text-xl">Fine-Tuning</h2>
                    <div className="flex items-center gap-2 mt-1">
                        <div className={`w-2 h-2 rounded-full ${analysis.confidenceScore > 90 ? 'bg-green-500' : 'bg-amber-500'}`}></div>
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                            {analysis.confidenceScore}% Confidence Rating
                        </span>
                    </div>
                </div>
                <div className="bg-blue-50 text-blue-600 p-2 rounded-lg" title="AI Assistant Active"><Sparkles size={20} /></div>
              </div>
              ...
              {activeStep === 3 ? (
                <div className="space-y-6">
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        Quantized Palette <Info size={10} />
                    </label>
                    <div className="flex gap-3">
                        {analysis.suggestedColors.map((color, idx) => (
                            <div key={idx} className="group relative">
                                <div className="w-10 h-10 rounded-xl border border-slate-100 shadow-sm transition-transform hover:scale-110 active:scale-95 cursor-help" style={{backgroundColor: color}}></div>
                                <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 hidden group-hover:block bg-slate-900 text-white text-[8px] px-1.5 py-0.5 rounded-full uppercase whitespace-nowrap z-50">{color}</div>
                            </div>
                        ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                      <div>
                        <div className="flex justify-between mb-2">
                            ...
                            <span className="text-xs font-bold px-2 py-1 bg-slate-100 rounded-md">{logoConfig.scale} mm</span>
                        </div>
                      ...
                      <div>
                        <div className="flex justify-between mb-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Rotation Angle</label>
                            <span className="text-xs font-bold px-2 py-1 bg-slate-100 rounded-md">{Math.round((logoConfig.rotation * 180) / Math.PI)}°</span>
                        </div>
                  ...
                  <div className="p-4 bg-orange-50 border border-orange-100/50 rounded-2xl">
                    <p className="text-[10px] leading-relaxed text-orange-800 font-medium">
                        <span className="font-bold uppercase mr-1 text-orange-900 tracking-tighter italic">AI Tip:</span>
                        {analysis.reasoning}
                    </p>
                  </div>

                  <div className="pt-4 border-t border-slate-50">
                    <button onClick={() => setActiveStep(4)} className="w-full py-5 bg-slate-900 text-white rounded-2xl font-bold flex items-center justify-center gap-3 shadow-xl shadow-slate-200 hover:bg-slate-800 transition-all active:scale-[0.98]">
                      Final Summary <ArrowRight size={18} />
                    </button>
                  </div>
                </div>
              ) : (
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-slate-50 p-6 rounded-2xl space-y-4">
                    <div className="flex justify-between text-sm text-slate-500 font-medium"><span>Base Product (Premium PLA)</span><span>€14.99</span></div>
                    <div className="flex justify-between text-sm text-slate-500 font-medium"><span>Multi-Layer Filament Config</span><span>€{(analysis.estimatedPrice - 14.99).toFixed(2)}</span></div>
                    <div className="flex justify-between text-lg font-bold pt-4 border-t border-slate-200 text-slate-900 italic font-display"><span>Total Amount</span><span>€{analysis.estimatedPrice.toFixed(2)}</span></div>
                  </div>
                  
                  <button 
                    onClick={handleShopifyCheckout} 
                    disabled={redirectStatus !== 'idle'} 
                    className="w-full py-5 bg-orange-600 text-white rounded-2xl font-bold shadow-xl shadow-orange-100 flex items-center justify-center gap-3 hover:bg-orange-700 transition-all active:scale-[0.98] disabled:opacity-50"
                  >
                    {redirectStatus === 'idle' ? 'Secure Checkout' : <><Loader2 className="animate-spin" size={18} /> Processing...</>}
                  </button>
                  
                  <button onClick={() => setActiveStep(3)} className="w-full text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] hover:text-slate-600 transition-colors">← Refine Design</button>
                </div>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
              <div className="p-5 rounded-[2rem] bg-white border border-slate-100 shadow-soft group hover:border-orange-200 transition-all">
                  <div className="w-10 h-10 rounded-xl bg-orange-50 flex items-center justify-center text-orange-600 mb-4 group-hover:scale-110 transition-transform"><Sparkles size={18}/></div>
                  <h3 className="text-[11px] font-bold mb-1 uppercase tracking-tighter">Color Quantized</h3>
                  <p className="text-[10px] text-slate-400 leading-relaxed font-medium">Auto-mapped to a maximum of 4 printable layers.</p>
              </div>
              <div className="p-5 rounded-[2rem] bg-white border border-slate-100 shadow-soft group hover:border-blue-200 transition-all">
                  <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600 mb-4 group-hover:scale-110 transition-transform"><ShieldCheck size={18}/></div>
                  <h3 className="text-[11px] font-bold mb-1 uppercase tracking-tighter">Nozzle Verified</h3>
                  <p className="text-[10px] text-slate-400 leading-relaxed font-medium">FDM-simulated pathing for 0.4mm precision.</p>
              </div>
          </div>
        </div>
      </main>
